...
class SucursalController {
  ...
  async getAll(req: Request, res: Response) {
    try {
      ...
      const user = await userService.getProfileById(userId as string);
      if (!user) return sendError(res, "No auth", 401);

      let sucursales = null;

      if (user.sucursal_id == null) {
        sucursales = await sucursalService.getAll();
      } else {
        sucursales = await sucursalService.getById(user.sucursal_id);
      }

      if (!sucursales) {
        return sendError(res, "Ocurrio un error", 500);
      }
